...
// query table bahp
$queryBahp = query("SELECT * FROM tb_bahp");
...
<head>
	...
	<title>Data Berita Acara Hasil Pengadaan Langsung - SiKuDa BaJa</title>
...
<body>
	<div class="wrapper">
			...
			<div class="content">
				<div class="page-inner">
					<div class="page-header">
						<h4 class="page-title">Data Berita Acara Hasil Pengadaan Langsung</h4>
						<ul class="breadcrumbs">
							...
							<li class="nav-item">
								<a href="#">Data Berita Acara Hasil Pengadaan Langsung</a>
							</li>
						</ul>
					</div>
					<div class="row">
							...
							<div class="card">
								<div class="card-header">
									<div class="d-flex align-items-center">
										<a href="add-bahp" class="btn bg-skb-3 text-white btn-round"><i class="fa fa-plus"></i> Buat BAHP</a>
										<h4 class="card-title ml-auto"><?php echo "{$resultBAHP["jmlhBAHP"]}"; ?> Data BAHP</h4>
									</div>
								</div>
								<div class="card-body">
											...
											<tbody>
												...
												<?php foreach ($queryBahp as $row) : ?>
													<tr>
															...
															<div class="form-button-action">
																<div class="dropdown show">
																	...
																	<div class="dropdown-menu dropdown-menu-opsi" aria-labelledby="dropdownMenuLink">
																		<a class="dropdown-item text-light bg-skb-3" href="edit-bahp?id_bahp=<?= $row["id"]; ?>"><i class="fa fa-edit"></i> Edit</a>
																		...
																		<form action="print-word" method="post">
																			...
																			<input type="text" hidden name="kode_rup" value="<?= $row["kode_rup"]; ?>">
																			<input type="text" hidden name="harga_penawaran" value="<?= number_format($row["harga_penawaran"], 0, ',', '.'); ?>">
																			<input type="text" hidden name="harga_penawaran_terbilang" value="<?= terbilang($row["harga_penawaran"]); ?>">
																			<input type="text" hidden name="harga_negosiasi" value="<?= number_format($row["harga_negosiasi"], 0, ',', '.'); ?>">
																			<input type="text" hidden name="harga_negosiasi_terbilang" value="<?= terbilang($row["harga_negosiasi"]); ?>">
																			<input type="text" hidden name="selisih" value="<?= number_format($row["harga_penawaran"] - $row["harga_negosiasi"], 0, ',', '.'); ?>">
																			<input type="text" hidden name="selisih_terbilang" value="<?= terbilang($row["harga_penawaran"] - $row["harga_negosiasi"]); ?>">
																		...
																		<a class="dropdown-item text-light bg-skb-6" href="#" data-toggle="modal" data-target="#delete-bahp<?= $row["id"]; ?>"><i class=" fa fa-trash"></i> Hapus</a>
														...
														<td>
															<?= substr($row["tgl_surat"], -5, 2); ?>.<?= substr($row["tgl_surat"], -2, 2); ?>/BAHP/<?= $row["no_kegiatan"]; ?>/PP/<?php if ($month01 === substr($row["tgl_surat"], -5, 2)) : ?><?php endif; ?><?php if ($month02 === substr($row["tgl_surat"], -5, 2)) : ?>II<?php endif; ?><?php if ($month03 === substr($row["tgl_surat"], -5, 2)) : ?>III<?php endif; ?><?php if ($month04 === substr($row["tgl_surat"], -5, 2)) : ?>IV<?php endif; ?><?php if ($month05 === substr($row["tgl_surat"], -5, 2)) : ?>V<?php endif; ?><?php if ($month06 === substr($row["tgl_surat"], -5, 2)) : ?>VI<?php endif; ?><?php if ($month07 === substr($row["tgl_surat"], -5, 2)) : ?>VII<?php endif; ?><?php if ($month08 === substr($row["tgl_surat"], -5, 2)) : ?>VIII<?php endif; ?><?php if ($month09 === substr($row["tgl_surat"], -5, 2)) : ?>IX<?php endif; ?><?php if ($month10 === substr($row["tgl_surat"], -5, 2)) : ?>X<?php endif; ?><?php if ($month11 === substr($row["tgl_surat"], -5, 2)) : ?>XI<?php endif; ?><?php if ($month12 === substr($row["tgl_surat"], -5, 2)) : ?>XII<?php endif; ?>/<?= substr($row["tgl_surat"], 0, 4); ?>
														</td>
														<td>
															<?= $row["hari_terbilang"]; ?>,
															<?= $row["tgl_terbilang"]; ?>,
														...
														<td>Rp. <?= number_format($row["harga_negosiasi"], 0, ',', '.'); ?></td>
														<td>
															Rp.
															<?= number_format($row["harga_penawaran"] -
																$row["harga_negosiasi"], 0, ',', '.');
															?>
														</td>
		...
		<!-- Modal Delete bahp -->
		<?php foreach ($queryBahp as $row) : ?>
			<div class="modal fade" id="delete-bahp<?= $row["id"]; ?>" tabindex=" -1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
				<div class="modal-dialog modal-dialog-centered" role="document">
					<div class="modal-content">
						<div class="modal-header">
							<h5 class="modal-title" id="exampleModalLongTitle">Hapus BAHP?</h5>
						...
						<div class="modal-footer">
							...
							<a href="delete-bahp?id_del_bahp=<?= $row["id"]; ?>" type="button" class="btn bg-skb-1 text-white">Hapus</a>
						</div>
					</div>
				</div>
			</div>
